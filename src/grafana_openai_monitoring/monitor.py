...
def __send_logs(logs_url, logs_username, access_token, logs):
    try:
        ...
        raise requests.exceptions.RequestException(f"Error sending Logs: {e}")
...
def __send_metrics(metrics_url, metrics_username, access_token, metrics):
    try:
        ...
        raise requests.exceptions.RequestException(f"Error sending Metrics: {e}") 

# Decorator function to monitor chat completion
def chatV2(func, metrics_url, logs_url, metrics_username, logs_username, access_token):
    ...
    def wrapper(*args, **kwargs):
        ...
        loki_response = __send_logs(logs_url=logs_url, logs_username=logs_username, access_token=access_token, logs=logs)
        ...
        prometheus_response = __send_metrics(metrics_url=metrics_url, metrics_username=metrics_username, access_token=access_token, metrics=metrics)
